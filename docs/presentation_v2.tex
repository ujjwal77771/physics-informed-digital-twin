  ...
  \begin{columns}[T]
    \column{0.56\textwidth}
      \begin{itemize}
        ...
        \item<2-> \textbf{Standard benchmark.} NASA C-MAPSS is the established
              PHM community testbed for turbofan degradation --- results
              are directly comparable with published literature.
    ...
    \column{0.41\textwidth}
      \begin{block}{NASA C-MAPSS at a glance}
        \small
        \begin{itemize}
          \item 3 sub-datasets: FD001, FD002, FD003
...
\begin{frame}{Results --- Two-Run Ablation Comparison}
  ...
  {\scriptsize
    ...
    \textit{Key finding: the Full model's range (3.45 cycles) is 25$\times$ narrower than Data-Only (88.68). Physics constraints prevent catastrophic failure.}
  }
\end{frame}
...
\begin{frame}{Results --- What Can and Cannot Be Claimed}
  \begin{columns}[T]
    ...
    \column{0.49\textwidth}
      ...
      \end{alertblock}
      \vspace{8pt}
      {\small\textbf{Future:} tune baseline; grid-search $\lambda$;
      conformal prediction intervals; extend to bearing datasets;
      pursue publication with your guidance.}
  \end{columns}
\end{frame}
