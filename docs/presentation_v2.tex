...
\begin{frame}{Results --- What Can and Cannot Be Claimed}
  \begin{columns}[T]
    ...
    \column{0.49\textwidth}
      ...
      \end{alertblock}
      \vspace{4pt}
      {\scriptsize\textbf{Key open direction --- UQ:}
      All results are point estimates (RMSE). Risk-aware maintenance
      requires a calibrated predictive distribution over RUL ---
      a maintenance engineer needs to know \emph{how confident} the estimate is,
      not just the point value. Bayesian inference or conformal prediction
      over this physics-informed baseline is the direction I most want to
      pursue in collaboration.}
  \end{columns}
\end{frame}
