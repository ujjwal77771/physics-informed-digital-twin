  ...
  \begin{columns}[T]
    \column{0.56\textwidth}
      \begin{itemize}
        ...
        \item<2-> \textbf{Harder-than-standard benchmark.} NASA C-MAPSS is the established
              PHM testbed. Standard literature trains and tests \emph{within} each sub-dataset.
              I use a cross-sub-dataset leave-one-out protocol --- training on two sub-datasets,
              testing on an unseen third. This is a domain-generalisation task, not in-domain
              accuracy, and is a strictly harder evaluation than the published benchmarks.
        \item<3-> \textbf{Deployability gap.} A maintenance engineer cannot act
              ...
              Physical consistency is \emph{mandatory}, not optional, for
              deployment trust.
        \item<4-> \textbf{Reproducibility.} The full pipeline is open-source
              with a live inference dashboard, making findings verifiable
              and extensible.
      \end{itemize}
    ...
    \column{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item<1-> A \textbf{patch-embedding Transformer encoder} processes 30-cycle windows
              of 14 C-MAPSS sensor channels --- exhaust temperatures (T24, T30, T50),
              pressures (P30, Ps30), fan/core speeds (Nf, Nc), bypass ratio, and bleed parameters.
        \item<2-> The training loss is augmented with \textbf{two physics-informed penalties}:
              a degradation-rate term and a monotonicity term.
        \item<3-> Each term is independently toggleable, enabling a clean ablation study.
        \item<4-> Inference is a standard forward pass --- no overhead at deployment time.
      \end{itemize}
      \vspace{6pt}
      \onslide<4->{%
      \[
...
\begin{frame}{The Original Contributions}
  \begin{enumerate}
    ...
    \item<1-> \textbf{Open-source physics-informed Transformer pipeline for NASA C-MAPSS.}\\
          {\small Preprocessing $\to$ training $\to$ ONNX export $\to$ FastAPI inference server
    ...
    \item<2-> \textbf{Ablation study isolating each physics term individually and jointly.}\\
          {\small All 5 variants (LSTM, Data-Only, Degradation-Only, Mono-Only, Full)
    ...
    \item<3-> \textbf{Transparent reporting of fold-collapse instability.}\\
          {\small The unconstrained Transformer collapsed on FD003 (RMSE\,99.39).
          This failure is reported and explained rather than discarded.}

    \item<4-> \textbf{Stability as the primary quantitative finding.}\\
          {\small Full model: fold-to-fold sample std $= 0.44$ cycles.
          Data-only: sample std $= 50.67$ cycles.
          On easy folds the physics constraints do not change accuracy ---
          they specifically prevent catastrophic failure on the hard fold (FD003),
          making the model reliably deployable across unseen sub-datasets.}
  \end{enumerate}
\end{frame}
...
\begin{frame}{Applications}
  \begin{center}
  % resizebox ensures the whole grid fits the frame width automatically
  \resizebox{0.97\textwidth}{!}{%
  \begin{tikzpicture}[every node/.style={inner sep=0pt}]
    % Row 1  (centres at y=0)
    \node (c1) at (0,   0) {\appcard{TealDk}{$\mathbf{\Uparrow}$}%
...
\begin{frame}{Results --- What Can and Cannot Be Claimed}
  \begin{columns}[T]
    ...
    \column{0.49\textwidth}
      ...
      \end{alertblock}
      \vspace{8pt}
      {\small\textbf{Key open direction --- Uncertainty Quantification:}
      All results here are point estimates (RMSE). For risk-aware
      maintenance decisions, a calibrated predictive distribution over RUL
      is essential --- a maintenance engineer needs to know not just
      \emph{when} the engine will fail, but \emph{how confident} that
      estimate is. Extending this physics-informed framework with Bayesian
      inference or conformal prediction intervals is the natural next step,
      and is the direction I most want to pursue in collaboration.}
  \end{columns}
\end{frame}

% ============================================================
\begin{frame}[standout]
  Thank you, Prof.\ Zio.\\[10pt]
  {\normalsize Looking forward to your feedback.}\\[12pt]
  {\small
